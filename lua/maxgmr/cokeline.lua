...
local components_old = {
	...
	unsaved = {
		text = function(buffer)
			return buffer.is_modified and "M" .. " " or ""
		end,
		fg = function(buffer)
...
cokeline.setup({
	...
	default_hl = {
		...
		fg = function(buffer)
			return buffer.is_focused and get_hl_attr("ColorColumn", "bg") -- old = Normal fg
		...
		bg = function(buffer)
			return buffer.is_focused and get_hl_attr("Normal", "fg") or get_hl_attr("ColorColumn", "bg")
		end,
